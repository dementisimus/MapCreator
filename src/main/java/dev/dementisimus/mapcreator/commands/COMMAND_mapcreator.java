 ...
 */
@OptionalBukkitCommand(name = "mapcreator", nameAliases = {"mc"})
public class COMMAND_mapcreator implements CommandExecutor {
            ...
            if(argsLength == 0 || argsLength == 1 || argsLength == 2) {
                if(argsLength == 1) {
                    ...
                    if(args[0].equalsIgnoreCase("set") && args[1].equalsIgnoreCase("spawn")) {
                        LocationManager.setLocation("SPAWN", player.getLocation(), b -> this.sendLocationSetMessage(player));
                        ...
                        if(subFolder.exists()) {
                            ...
                            player.sendMessage(new BukkitTranslation(Translations.CREATOR_COMMAND_TREE_SUBFOLDER_DOES_NOT_EXIST).get(player, new String[]{"$prefix$", "$folder$"}, new String[]{Prefixes.MAPCREATOR, args[1].toUpperCase()}));
                        }
                        ...
                        if(arg.equalsIgnoreCase("all")) {
                            ...
                            for(Player pl : Bukkit.getOnlinePlayers()) {
                                if(!pl.getName().equalsIgnoreCase(player.getName())) {
                                    this.teleport(player, pl, pl, Translations.CREATOR_TELEPORTED_TO_WORLD_BY_PLAYER);
                                    if(count == 0) {
                                        firstP = pl;
                                    }
                                    count++;
                            ...
                            if(count == 0) {
                                player.sendMessage(new BukkitTranslation(Translations.CREATOR_NO_PLAYER_TELEPORTED).get(player, "$prefix$", Prefixes.MAPCREATOR));
                            }else {
                                if(count == 1) {
                                    player.sendMessage(new BukkitTranslation(Translations.CREATOR_PLAYER_TELEPORTED).get(player, new String[]{"$prefix$", "$player$"}, new String[]{Prefixes.MAPCREATOR, firstP.getName()}));
                                }else {
                                    player.sendMessage(new BukkitTranslation(Translations.CREATOR_PLAYERS_TELEPORTED).get(player, new String[]{"$prefix$", "$playerCount$", "$players$"}, new String[]{Prefixes.MAPCREATOR, String.valueOf(count), playersTeleported}));
                                }
                            }
                        }else if(players != null && !players.isEmpty()) {
                            Player target = players.get(0);
                            if(!target.getName().equalsIgnoreCase(player.getName())) {
                                this.teleport(player, player, player, Translations.CREATOR_TELEPORTED_TO_WORLD);
                                target.sendMessage(new BukkitTranslation(Translations.CREATOR_PLAYER_TELEPORTED).get(player, new String[]{"$prefix$", "$player$"}, new String[]{Prefixes.MAPCREATOR, target.getName()}));
                            }else {
                                this.sendHelpMessage(player);
                            }
                            ...
                            if(Bukkit.getWorld(arg) != null) {
                                ...
                                player.sendMessage(new BukkitTranslation(Translations.CREATOR_WORLD_NOT_LOADED_MAP_ONLY).get(player, new String[]{"$prefix$", "$map$"}, new String[]{Prefixes.MAPCREATOR, arg}));
                            }
                        }else {
                            this.sendHelpMessage(player);
                        }
                ...
                if(argsLength <= 4) {
                    ...
                    if(argsLength == 3) {
                        if(workType.equalsIgnoreCase("LOAD")) {
                            this.loadWorld(player, mapName, creator, false);
                        ...
                        if(workType.equalsIgnoreCase("LOAD")) {
                            this.loadWorld(player, mapName, creator, useDefaultWorldSettings);
                        }else {
                            this.sendHelpMessage(player);
                        }
                    }
    ...
    private void teleport(Player player, Player target, Player sendMessageTo, String translations) {
    ...
    private void loadWorld(Player player, String world, MapCreatorAPI creator, boolean setDefaultWorldSettings) {
        if(!player.getWorld().getName().equalsIgnoreCase(world)) {
            ...
            player.sendMessage(new BukkitTranslation(Translations.CREATOR_WORLD_ALREADY_LOADED_NAME_ONLY).get(player, new String[]{"$prefix$", "$map$"}, new String[]{Prefixes.MAPCREATOR, world}));
        }
    }

    private void sendHelpMessage(Player player) {
        player.sendMessage(new BukkitTranslation(Translations.COMMAND_MAPCREATOR_HELP).get(player, new String[]{"$prefix$"}, new String[]{Prefixes.MAPCREATOR}));
    }

    private void sendLocationSetMessage(Player player) {
        player.sendMessage(new BukkitTranslation(Translations.COMMAND_MAPCREATOR_SET_SPAWN).get(player, new String[]{"$prefix$", "$world$"}, new String[]{Prefixes.MAPCREATOR, player.getWorld().getName()}));
    }
